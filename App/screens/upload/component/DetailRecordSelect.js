...
const DetailRecordSelect = ({title, onSaveMyOutfits, myOutfits}) => {
  const [isModalOpen, setIsModalOpen] = useState(false);

  const handleSelectedIcon = () => {
    if (myOutfits?.score === 0) {
      return images.face_bad;
    } else if (myOutfits?.score === 1) {
      return images.face_soso;
    } else if (myOutfits?.score === 2) {
    ...
    <>
      ...
      <WeatherModal isModalOpen={isModalOpen} onSetModal={setIsModalOpen}>
        <View style={styles.bottomSheetContainer}>
          ...
          <DetailInputArea>
            <DetailSelectWrapper onPress={() => onSaveMyOutfits(0)}>
              <Image style={styles.icon} source={images.face_bad} />
              <OutfitChoice
                choicedBackground={myOutfits?.score === 0 ? 'black' : 'white'}>
                <ChoiceText
                  choicedFontColor={myOutfits?.score === 0 ? 'white' : 'black'}>
                  별로예요
                </ChoiceText>
              </OutfitChoice>
            </DetailSelectWrapper>

            <DetailSelectWrapper onPress={() => onSaveMyOutfits(1)}>
              <Image style={styles.icon} source={images.face_soso} />
              <OutfitChoice
                choicedBackground={myOutfits?.score === 1 ? 'black' : 'white'}>
                <ChoiceText
                  choicedFontColor={myOutfits?.score === 1 ? 'white' : 'black'}>
                  보통이에요
                </ChoiceText>
              </OutfitChoice>
            </DetailSelectWrapper>

            <DetailSelectWrapper onPress={() => onSaveMyOutfits(2)}>
              <Image style={styles.icon} source={images.face_good} />
              <OutfitChoice
                choicedBackground={myOutfits?.score === 2 ? 'black' : 'white'}>
                <ChoiceText
                  choicedFontColor={myOutfits?.score === 2 ? 'white' : 'black'}>
                  완전 찰떡!
                </ChoiceText>
              </OutfitChoice>
            </DetailSelectWrapper>
